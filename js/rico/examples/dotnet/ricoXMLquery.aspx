...
Sub Page_Load(Sender As object, e As EventArgs)
  ...
  elseif not IsNumeric(RequestOffset) then
    XmlObj.ErrorMsg="Invalid offset!"
  elseif not IsNumeric(RequestSize) then
  ...
  elseif not app.OpenDB() then
    ...
    XmlObj.offset=CLng(RequestOffset)
    XmlObj.numrows=CLng(RequestSize)
    XmlObj.filters=session.contents(RequestId & ".filters")
    Dim ParseObj=new sqlParse()
    ParseObj.ParseSelect(session.contents(RequestId))
    XmlObj.oParse=ParseObj
    XmlObj.dbConnection=app.dbConnection
    XmlObj.dbDialect=app.dbDialect
    XmlObj.sendDebugMsgs=true
    XmlObj.LogSqlOnError=true
