...
initSheet: function() {
  ...
  if (this.menu) this.registerMenu(menu);
...
registerMenu: function(menu) {
  menu.showmenu=this.showSheetMenu;
  menu.showSubMenu=this.showSheetSubMenu;
  menu.grid=this;
  menu.createDiv(this.scrollDiv);
},

/**
 * Override for menu.showmenu().
 * Warning: "this" refers to menu object, not grid object.
 */
showSheetMenu: function(e,hideFunc) {
  ...
  this.div.style.top=Math.floor(elem.offsetTop+elem.offsetHeight/2)+"px";
  this.div.style.left=Math.floor(td.offsetLeft+td.offsetWidth/2)+"px";
...
showSheetSubMenu: function(a,submenu) {
  ...
  submenu.div.style.top=(top+a.offsetTop)+"px";
  submenu.div.style.left=(left+a.offsetWidth)+"px";
...
showInputArea: function(clear,e) {
  ...
  if (clear) {
    ...
    if (this.inputIdx.cell.RicoFormula)
      this.inputArea.value=this.inputIdx.cell.RicoFormula.toString();
    else
