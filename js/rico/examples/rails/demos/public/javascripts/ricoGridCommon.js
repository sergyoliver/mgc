...
Rico.GridCommon.prototype = {
  ...
  getCookie: function(itemName) {
    return RicoUtil.getCookie(this.tableId+'.'+itemName);
  },
  
  // Write information to cookie
  setCookie: function(itemName,itemValue) {
  	RicoUtil.setCookie(this.tableId+'.'+itemName,itemValue,this.options.cookieDays,this.options.cookiePath,this.options.cookieDomain);
  }
...
Rico.TableColumn.prototype = {
  ...
  baseInit: function(liveGrid,colIdx,hdrInfo,tabIdx) {
    ...
    if (liveGrid.options.saveColumnInfo.width) {
      var c=this.getCookie("width");
      Rico.writeDebugMsg("TableColumn.init index="+colIdx+" width-cookie="+c);
      if (c=='hidden')
        this.visible=false;
    ...
    if (liveGrid.options.saveColumnInfo.sort) {
      var c=this.getCookie("sort");
      Rico.writeDebugMsg("TableColumn.init index="+colIdx+" sort-cookie="+c);
      if (c==Rico.TableColumn.SORT_ASC || c==Rico.TableColumn.SORT_DESC)
        this.setSorted(c);
    }
    if (liveGrid.options.saveColumnInfo.filter) {
      var op=this.getCookie("filter","op");
      var len=this.getCookie("filter","len");
      if (op && len) {
        Rico.writeDebugMsg("TableColumn.init index="+colIdx+" filter-cookie="+op);
        this.filterValues = [];
        this.filterType = Rico.TableColumn.USERFILTER;
        this.filterOp = op;
        len=parseInt(len);
        for (var i=0; i<len; i++)
          this.filterValues.push(this.getCookie("filter",i));
      }
    }
  ...
  handleMouseUp: function(e) {
    ...
    this.setCookie('width',this.colWidth);
  ...
  setSorted: function(direction) {
    ...
    this.setCookie('sort',direction);
  },

  cookieName: function(param,subkey) {
    var n=this.fieldName+"."+param;
    if (subkey) n+='.'+subkey;
    return n;
  },
  ...
  // Gets the value of the specified cookie
  // subkey is optional
  getCookie: function(itemName,subkey) {
    return this.liveGrid.getCookie(this.cookieName(itemName,subkey));
  },
  
  // Write information to cookie
  // subkey is optional
  setCookie: function(itemName,itemValue,subkey) {
    if (!this.options.saveColumnInfo[itemName]) return;
    return this.liveGrid.setCookie(this.cookieName(itemName,subkey),itemValue);
  }
