...
Rico.Buffer.Base.prototype = {
  ...
  isVisible: function(bufRow) {
    return bufRow >= this.windowStart && bufRow < this.windowEnd;
  },
...
setUnfiltered: function() {
  ...
  this.setCookie('filter',0,'len');
  this.setCookie('filter','','op');
...
addFilterNE: function() {
  ...
  this.setCookie('filter',this.filterValues.length,'len');
  this.setCookie('filter',this.userFilter,this.filterValues.length-1);
  if (this.options.filterHandler)
    this.options.filterHandler();
},
...
setFilter: function(relop,filter,type,removeFilterFunc) {
  ...
  this.filterOp = relop;
  if (type == Rico.TableColumn.USERFILTER) {
    this.setCookie('filter',1,'len');
    this.setCookie('filter',relop,'op');
    this.setCookie('filter',filter,0);
  }
