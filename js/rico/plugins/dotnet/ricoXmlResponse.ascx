...
Protected Overrides Sub Render(writer as HTMLTextWriter)
  ...
  elseif IsNothing(dbConnection) then
    writer.Write("<rows update_ui='false' /><error>No database connection</error>")
  else
    try
      RenderRows(writer)
    Catch ex As Exception
      writer.Write("<rows update_ui='false' /><error>" & server.HTMLEncode(ex.Message))
...
Protected Sub RenderRows(writer as HTMLTextWriter)
  dim rowcnt as integer=0, fldNum as integer, dbDate as DateTime, strFieldItem as String
  ...
  if IsNothing(sqlText) then exit sub
  DebugMsgs.add(sqlText)
  DebugMsgs.add(dbClassName)
  DebugMsgs.add(dbVersion)
  dim totcnt as integer=0, rdr as object
  ...
  if eof then
    writer.WriteLine("<rowcount>" & totcnt & "</rowcount>")
  end if
